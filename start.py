...
def find_ardublockly_dir(search_path):
    """Find the DB4K project directory absolute path.

    Navigates within each node of given path and tries to find the DB4K
    project root directory. Assumes that the project root will have an folder
    name DB4K with an index.html file inside.
    ...
    :param search_path: Path starting point to search the DB4K project
            root folder.
    :return: Path to the DB4K root folder. If not found returns None.
    """
    path_to_navigate = os.path.normpath(search_path)
    # Navigate through each path node from the bottom up
    while path_to_navigate:
        # Check if file DB4K/index.html exists within current path
        if os.path.isfile(
                os.path.join(path_to_navigate, 'DB4K', 'index.html')):
    ...
    if len(sys.argv) == 1:
        ...
        for opt, arg in opts:
            ...
            if opt in ('-s', '--serverroot'):
	         ...
	         #**********************************************************************
			 # Modified by RLQ DB4K  changed Ardublockly by DB4K 
                print('Parsed "%s" flag. The DB4K project root will be '
...
def main():
	...
	#**********************************************************************
	# Modified by RLQ DB4K  changed the root folder Ardublockly by DB4K 
	
    # Based on command line options, set the server root to the DB4K
    ...
    if ardublockly_root_dir is None:
        print('The DB4K project root folder could not be found within '
              'the %s directory !' % this_file_dir)
        sys.exit(1)
    print('DB4K root directory:\n\t%s' % ardublockly_root_dir)
    ...
    if find_project_root is True or server_root is None:
        ...
        # Arguments have set a server root, and to not find DB4K dir
        if not os.path.commonprefix([server_root, ardublockly_root_dir]):
            print('The DB4K project folder needs to be accessible from '
